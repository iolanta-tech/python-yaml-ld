...
def test_pyld_expand_json_string():
    ...
    with pytest.raises(jsonld.JsonLdError) as error_info:
...
def test_https():
    response = yaml_ld.expand(
        'https://w3c.github.io/yaml-ld/tests/manifest.jsonld',
    )

    assert response == [
        IsPartialDict({
            '@id': 'https://w3c.github.io/yaml-ld/tests/basic-manifest',
        }),
    ]
